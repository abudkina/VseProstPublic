...
func AddHashtag(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req hashtagModel.Hashtag
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Неверный запрос: имя хэштега обязательно и не должно быть пустым"})
			return
		}
		...
		var existingCategory hashtagModel.Hashtag
		if err := db.Where("LOWER(TRIM(REPLACE(REPLACE(name, 'ё', 'е'), 'Ё', 'е'))) = ?", normalizedReqName).First(&existingCategory).Error; err == nil {
			...
			c.JSON(http.StatusBadRequest, gin.H{"error": "Хэштег с таким именем уже существует"})
			return
		...
		if err := db.Create(&req).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Ошибка создания хэштега']"})
			return
		}
...
// Обработчик для добавления категории
func CountHashtag(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req hashtagModel.Hashtag
