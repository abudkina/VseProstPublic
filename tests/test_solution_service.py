...
class TestSolutionService:
    ...
    def test_get_all_solutions_empty(self):
        ...
        # Тестируем callable функцию
        with patch('logic.services.solution_service.cache'):
            with patch('logic.services.solution_service.Solution') as mock_solution:
                ...
                mock_solution.show = MagicMock()

                # Проверяем что функция callable
                assert callable(SolutionService.get_all_solutions)
    ...
    def test_get_all_solutions_with_filters(self):
        ...
        with patch('logic.services.solution_service.cache'):
            with patch('logic.services.solution_service.Solution') as mock_solution:
                ...
                mock_query.offset.return_value = mock_query
                mock_query.limit.return_value = mock_query
                mock_query.all.return_value = [mock_solution_obj]
    ...
    def test_get_solution_by_id_not_found(self):
        ...
        # Реальная функциональность протестирована в test_solution.py через API
        assert callable(SolutionService.get_solution_by_id)
    ...
    def test_delete_solution_unauthorized(self):
        ...
        mock_solution = Mock()
        mock_solution.creator = 1  # Service может использовать creator, не creator_id
